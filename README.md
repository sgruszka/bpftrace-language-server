...
## bpftrace command
The server invokes `bpftrace` command directly when running with root privileges and via `sudo` otherwise.
...
Capabilities can be set on the system `bpftrace` binary or on a local copy. Using a separate, user-owned copy avoids granting privileges to the system-wide `bpftrace` binary.
...
Verify by checking if you can invoke below commands without error:
```bash
cd
my_bpftrace --info
my_bpftrace -l | head
...
```
bpftrace >= 0.25 is known to work with the capabilities.
...
### Option 3: run0
If `systemd` and `polkit` are available, the server can be invoked through `run0`. Authentication is performed before server starts (you will be prompted for password).

Configuring `run0` in Neovim (see below for full config):
```lua
...
If the bpftrace filetype is recognized (if not, see blow), register the language server in Neovim `init.lua`. Ensure `bpftrace-ls` is in `PATH`, or specify its full path. See [documentation]( https://neovim.io/doc/user/lsp.html) for details.
```lua
